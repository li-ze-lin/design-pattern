 ...
 *
 *	一、什么是职责链模式:
 *		Chain of Responsibility（CoR）模式也叫职责链模式或者职责连锁模式，
 *		是行为模式之一，该模式构造一系列分别担当不同的职责的类的对象来共同完成一个任务，
...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.println("---------------");
		CarHeadHandler handler = new CarHeadHandler();
		handler.setNextHandler(new CarBodyHandler()).setNextHandler(new CarTailHandler());
		handler.HandlerCar();
	}
